...
- **Categories** hold a set of fields (e.g. Thanksgiving → Name, Phone Number, Email). The
  homepage (`index.html`) lists every category that's been created, so anyone visiting the
  site can see and open any form — this is an open directory, not a private-link-only model.
...
## 1. Set the admin password and Blobs credentials
...
| Key | Value | Why |
|---|---|---|
| `ADMIN_PASSWORD` | your chosen password | Falls back to `Psalm119:105` in code if unset |
| `SITE_ID` | your site's ID | Required for Netlify Blobs to work reliably in Functions |
| `BLOBS_TOKEN` | a Personal Access Token | Required alongside `SITE_ID` for Blobs |

**Where to find these:**
- `SITE_ID`: **Site configuration → General → Site details → Site ID** (a UUID, not the site name)
- `BLOBS_TOKEN`: create one under your **Netlify user avatar → User settings → Applications → Personal access tokens → New access token**. Give it a name like `collation-ledger-blobs` and no expiry (or a long one).

This mirrors the same fix needed on the leave app — some Netlify environments don't
automatically inject Blobs credentials into Functions, causing a
`MissingBlobsEnvironmentError` crash. Setting `SITE_ID` and `BLOBS_TOKEN` explicitly avoids
that regardless of environment.

**If you skip `SITE_ID`/`BLOBS_TOKEN`:** the app will still try Netlify's automatic Blobs
context first. If that crashes with `MissingBlobsEnvironmentError`, you'll need to set these
two variables.
